...
def generate_qa_pairs(info_path: str, view_index: int, img_width: int = 150, img_height: int = 100) -> list:
    ...
    for kart in karts:
        if kart["name"] == ego_kart:
            continue
        rel = kart.get("relative_position", "unknown")
        question = f"Where is {kart['name']} relative to the ego car?"
        qa_pairs.append({
            "question": question,
            "answer": rel,
            "image_file": get_image_filename(info_path, view_index)
...
def get_image_filename(info_path: str, view_index: int) -> str:
    ...
    # Relative path for dataset consistency (e.g., "train/xxx.jpg")
    return f"{info_path.parent.name}/{image_file}"
